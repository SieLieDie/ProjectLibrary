...
public class BookDAO {
    private static final String RETURN_ALL_BOOK = "SELECT book_id, book_name, concat(author_first_name,\" \", author_second_name) author_name, " +
            "genre_name, in_stoke, description " +
    ...
    private static final String RETURN_BOOK_BY_GENRE = "SELECT book_id, book_name, concat(author_first_name,\" \", author_second_name) author_name," +
            " genre_name, in_stoke, description FROM book JOIN book_bounds_author USING (book_id) JOIN book_author USING (author_id) " +
            "JOIN book_bounds_genre USING (book_id) JOIN book_genre USING (genre_id) " +
    ...
    private static final String ADD_BOOK = "INSERT INTO book (book_name, in_stoke, description) VALUES (?, ?, ?)";
    ...
    public void returnAllBookFromBD(List<Book> bookList, int langID, Integer userID) {
        try {
            returnBookFromBD(bookList, langID, RETURN_ALL_BOOK);
            List<Book> bookFavoriteList = new ArrayList<>();
            if (userID != null) {
                showFavorite(bookFavoriteList, langID, userID);
                markFavoriteBook(bookList, bookFavoriteList);
            }
        } catch (SQLException e) {
            EXCEPTION_LOG.info("SQLException in DAOMethod 'returnAllBook'\n" + e);
        }
    }

    private void returnBookFromBD(List<Book> bookList, int langID, String sql) throws SQLException {
    ...
    private void markFavoriteBook(List<Book> bookList, List<Book> bookFavoriteList) {
        ...
        try (ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                bookList.add(new Book(resultSet.getInt(BOOK_ID), resultSet.getString("book_name"),
                        resultSet.getString("author_name"), resultSet.getString("genre_name"), resultSet.getInt("in_stoke"),
                        resultSet.getString("description"), false));
            }
        }
    }

    public void returnBookByGenre(List<Book> bookList, int genreID, int langID, Integer userID) {
        try {
            returnBookListForAttribute(bookList, genreID, langID, RETURN_BOOK_BY_GENRE);
            List<Book> bookFavoriteList = new ArrayList<>();
            if (userID != null) {
                showFavorite(bookFavoriteList, langID, userID);
                markFavoriteBook(bookList, bookFavoriteList);
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'returnBookByGenre'\n" + e);
        }
    }

    public void returnBookByAuthor(List<Book> bookList, int authorID, int langID, Integer userID) {
        try {
            returnBookListForAttribute(bookList, authorID, langID, RETURN_BOOK_BY_AUTHOR);
            List<Book> bookFavoriteList = new ArrayList<>();
            if (userID != null) {
                showFavorite(bookFavoriteList, langID, userID);
                markFavoriteBook(bookList, bookFavoriteList);
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'returnAllBookByAuthor'\n" + e);
        }
    }

    public void searchBook(List<Book> bookList, String bookName, int langID, Integer userID) {
        try {
            returnBookFromBD(bookList, langID, SEARCH_THE_BOOK.concat("'%").concat(bookName).concat("%'"));
            List<Book> bookFavoriteList = new ArrayList<>();
            if (userID != null) {
                showFavorite(bookFavoriteList, langID, userID);
                markFavoriteBook(bookList, bookFavoriteList);
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'searchBook'\n" + e);
        }
    }

    public void showFavorite(List<Book> bookList, int langID, int userID) {
        try {
            returnBookListForAttribute(bookList, userID, langID, RETURN_FAVORITE);
            for (Book book : bookList) {
                book.setIsFavorite(true);
            }
        } catch (SQLException e) {
            EXCEPTION_LOG.info("Exception in DAOMethod 'showFavorite'\n" + e);
        }
    }

    private void returnBookListForAttribute(List<Book> bookList, int attributeID, int langID, String sql) throws SQLException {
        ConnectionPull connectionPull = ConnectionPull.getInstance();
        Connection connection = connectionPull.retrieve();
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
    ...
    public void returnGenresForBook(List<Attribute> genreList, int langID) {
        try {
            returnAttributeForBook(genreList, RETURN_GENRES_FOR_BOOK + langID, GENRE_ID, "genre_name", "genre_core_id");
        } catch (SQLException e) {
            EXCEPTION_LOG.info("Exception in DAOMethod 'returnGenresForBook'\n" + e);
        }
    }

    public void returnAuthorForBook(List<Attribute> authorList, int langID) {
        try {
            returnAttributeForBook(authorList, RETURN_AUTHOR_FOR_BOOK + langID, AUTHOR_ID, "author_name", "author_core_id");
        } catch (SQLException e) {
            EXCEPTION_LOG.info("Exception in DAOMethod 'returnAuthorForBook'\n" + e);
        }
    }

    private void returnAttributeForBook(List<Attribute> attributeList, String sql, String attributeID, String attributeName, String attributeCoreID) throws SQLException {
        ConnectionPull connectionPull = ConnectionPull.getInstance();
        Connection connection = connectionPull.retrieve();
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public void removeBook(int bookID) {
        try {
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'removeBook'\n" + e);
        }
    }
    ...
    public void removeGenre(int genreCoreID) {
        try {
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'removeGenre'\n" + e);
        }
    }
    ...
    public void removeAuthor(int authorCoreID) {
        try {
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'removeAuthor'\n" + e);
        }
    }
    ...
    public void removeFromFavorite(int bookID, int userID) {
        ...
        try {
            ...
            EXCEPTION_LOG.info("Exception in DAOMethod 'removeFromFavorite'\n" + e);
        }
        connectionPull.putBack(connection);
    }
}
